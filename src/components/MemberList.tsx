import { formatPublishedAt } from "@/lib/format";
import type { StoryMember } from "@/lib/types";

type MemberListProps = {
  members: StoryMember[];
  outletCount: number;
};

export function MemberList({ members, outletCount }: MemberListProps) {
  return (
    <section aria-label={`Sources (${outletCount})`}>
      <div className="mb-2 mt-4 text-[10px] uppercase tracking-[0.1em] text-[var(--faint)]">
        Sources
      </div>
      <ul className="flex list-none flex-col gap-2.5 p-0">
        {members.map((m, i) => (
          <li
            key={`${m.canonical_url}-${i}`}
            className="rounded-sm border border-[var(--border)] bg-[var(--surface)] p-3 shadow-[var(--shadow-card)]"
          >
            <div className="text-[11px] font-bold uppercase tracking-[0.06em] text-[var(--violet)]">
              {m.outlet_name}
            </div>
            <h4 className="mt-1 text-[14px] font-semibold leading-snug text-[var(--ink)]">
              {m.title}
            </h4>
            {m.dek ? (
              <p className="mt-1.5 text-[13px] leading-relaxed text-[var(--ink-2)]">
                {m.dek}
              </p>
            ) : null}
            <div className="mt-2 flex flex-wrap items-center justify-between gap-2 text-[11px] text-[var(--faint)]">
              <span>{formatPublishedAt(m.published_at)}</span>
              <a
                href={m.canonical_url}
                target="_blank"
                rel="noopener noreferrer"
                className="font-semibold text-[var(--violet)] underline-offset-2 hover:underline"
              >
                Open source ↗
              </a>
            </div>
          </li>
        ))}
      </ul>
    </section>
  );
}
